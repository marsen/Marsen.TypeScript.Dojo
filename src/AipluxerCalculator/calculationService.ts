    ...
    const categoriesPriceDetail = categories.map(category => {
      ...
      const itemsDetail = [
        ...
        { name: PRODUCT_NAMES[4], 
          quantity: code === '35' && items.some(item => item.startsWith('3519')) ? excessQty : 0, 
          unitPrice: code === '35' && items.some(item => item.startsWith('3519')) && excessQty !== 0 ? this.specialServiceFee : 0, 
